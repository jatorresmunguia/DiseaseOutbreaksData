...
for(i in 2019){ # 
  ...
  data19 <- data.frame("Outbreak" = rep(NA, each = length(results)),
  ...
  for(j in 1:length(results)){ # Total number of outbreaks in the year 
    ...
    data19[j, "Date"] <- newsdate # Only text corresponding to full data19
    data19[j, "Month"] <- na.omit(as.data.frame(str_extract(tolower(monthtext), months)))[1, 1]
    data19[j, "Description"] <- text # All description
    data19[j, "Outbreak"] <- strsplit(text, "[–]")[[1]][1] # Only text corresponding to Outbreak
    data19[j, "Country"] <- strsplit(text, "[–]")[[1]][2] # Only text corresponding to Country Name
    data19[j, "Link"] <- paste0("https://www.who.int", linkfull) # Only text corresponding to full data19
  }
  
  data19$Outbreak <- trimws(gsub(data19$Outbreak, pattern = paste0(i, " - "), replacement = ""))
  data19$Country <- trimws(gsub(data19$Country, pattern = "\\(new outbreak\\)", replacement = ""))
  data19 <- data19[!grepl(pattern = "pdate", data19$Description), ]
  rownames(data19) <- 1:nrow(data19)
  assign(paste0("data", i), data19)
} # 119 DON´s related to 2019

### Extract all years except 2019 from data2
data2 <- data2[data2$Year != 2019, ] # 2639-2581 = 58
data2 <- rbind(data2, data2019) # 2700 DON's
...
data3 <- data3[order(data3$Year, data3$month_num, data3$Day, data3$Link, data3$Country), ]
...
write.csv(DONsOutbreaks, file = "DONsOutbreaks.csv")
...
write.csv(COVIDOutbreaks, file = "COVIDOutbreaks.csv")
